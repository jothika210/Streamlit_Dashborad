...
def fetch_distinct_rs_names():
    ...
    """
    return load_data_from_db(query)['rs_name'].tolist()
 
def fetch_sales_hierarchy(selected_rs_name):
    query = f"""
...
def fetch_sales_hierarchy_by_asm(selected_asm_name):
    query = f"""
        SELECT DISTINCT sh.cluster, sh.rs_name, sh.rs_code, sh.asm_emp_id,
...
def display_distributor_section():
    ...
    filter_method = st.sidebar.radio(
        "Choose filter",
        ("By Distributor Name", "By Cluster", "By ASM Name", "By Username")
    )
 
    if filter_method == "By Distributor Name":
        try:
            distinct_rs_names = fetch_distinct_rs_names()
            selected_rs_name = st.sidebar.selectbox("Distributor Name", distinct_rs_names, index=None, placeholder="Select a Distributor Name")
 
            if selected_rs_name:
                data = fetch_sales_hierarchy(selected_rs_name)
                ...
                if not data.empty:
                    ...
                    for survey_id in selected_ids:
                        details = fetch_response_details(survey_id)
                        ...
                        st.write(f"**Survey Details for {selected_rs_name} - {survey_id} ({username}):**")
 
                        if not details.empty:
                            matching_images = fetch_matching_images(survey_id, user_id)
                            ...
                            if not matching_images.empty:
                                ...
                                for idx, row in matching_images.iterrows():
                                    ...
                                    file_link = (f"https://distributorkpi.cavininfotech.com/api/viewimage/"
                                                 f"Administrative%20Capability/{blob_filename_encoded}")
            ...
            if selected_cluster_name != "Select a Cluster":
                ...
                SELECT DISTINCT sh.cluster, sh.rs_name, sh.rs_code, sh.asm_emp_id,
                       ...
                       FROM pathway.response_summary rs
                        ...
                        LEFT JOIN pathway.response_image ri ON rs.survey_submitted_id = ri.survey_submitted_id
                        LEFT JOIN pathway.user_data ud ON rs.userid = ud.empcode
                        WHERE rp.q_id in(9,11,13,16) AND sh.cluster = '{selected_cluster_name}'
                """
            ...
            if selected_username :
                ...
                SELECT DISTINCT sh.cluster, sh.rs_name, sh.rs_code, sh.asm_emp_id,
                       ...
                       FROM pathway.response_summary rs
                        ...
                        LEFT JOIN pathway.user_data ud ON rs.userid = ud.empcode
                        WHERE rp.q_id in(9,11,13,16)
                        AND ud.username = '{selected_username}'
                """
